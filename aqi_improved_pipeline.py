...
# Get sorted indices (same order SHAP uses)
sorted_idx = np.argsort(importance)[::-1][:20]

# Create ranking labels
rank_labels = [
    f"{i+1}. {FEATURE_COLS[idx]}"
    for i, idx in enumerate(sorted_idx)
]
plt.figure(figsize=(10, 8))
shap.summary_plot(shap_values, X_test_sample, feature_names=FEATURE_COLS, max_display=20, show=False)
plt.yticks(ticks=range(len(rank_labels)), labels=rank_labels[::-1])
...
# Save the scaler AND feature column list — critical for inference
joblib.dump(scaler_X,    "models/scaler_X.pkl")
joblib.dump(scaler_y,    "models/scaler_y.pkl")
joblib.dump(FEATURE_COLS,"models/feature_cols.pkl")
joblib.dump(le,          "models/label_encoder.pkl")  # City LabelEncoder

# Save historical data for lag feature construction
df.to_csv("models/historical_aqi.csv", index=False)
